...
import { NotionSyncResult, syncTaskToNotion, syncTodoToNotion } from '../lib/taskSync';
...
export const handleSyncBatch = defineHandler<SyncModeInput, NotionSyncResult, NotionConfig>(
  async (input, context) => {
    context.logger.info('Processing Notion sync batch', {
    ...
    // find a fresh todo→Notion-page mapping to relate time-log rows to.
    ...
    for (const change of orderedChanges) {
      ...
      try {
        let result: NotionSyncResult | null = null;

        if (change.entityType === 'todo') {
          result = await syncTodoToNotion(
            {
              event: `todo.${op}`,

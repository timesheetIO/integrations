...
- Manifest id: `datev`
- Category: accounting
- Data access: `documents`, `absences`, `overtime`, `tasks`, `colleagues` (all read only)
...
| `erloeskontoReverseCharge` | `8337` / `4337` | Revenue account for reverse charge invoices |
...
| `taggeldKeyword`, `naechtigungsgeldKeyword` | `Taggeld`, `Nächtigungsgeld` | Expenses whose description contains the keyword are summed as that allowance; empty disables it |
| `lohnartTaggeld`, `lohnartNaechtigungsgeld` | empty | Wage types for the two allowances; empty means the amounts are not exported |
...
- `build-payroll` `{ from, to }`: same input.
...
| BU-Schluessel | empty: the default revenue accounts are automatic accounts |
| Belegdatum | `TTMM` |
...
| Leistungsdatum, Faelligkeit | `deliveryDate`, `dueDate` as `TTMMJJJJ` |
...
`abrechnung_zeitraum` is the first day of the period. `bs_nr` is `1` for hours, `2` for days and `3` for amounts (`LODAS_BS_NR`).
...
- Travel allowances: expenses in the period whose description contains `taggeldKeyword` or `naechtigungsgeldKeyword` (case-insensitive) are summed per employee and written as one amount line per wage type (`lohnartTaggeld`, `lohnartNaechtigungsgeld`; LODAS `bs_nr` 3, Lohn und Gehalt `Betrag` column). A keyword that matches expenses without a configured wage type is reported once as `expense_lohnart_missing`.

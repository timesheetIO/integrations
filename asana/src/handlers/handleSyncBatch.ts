...
import { AsanaSyncResult, syncTimesheetTaskToAsana, syncTodoToAsana } from '../lib/taskSync';
...
export const handleSyncBatch = defineHandler<SyncModeInput, AsanaSyncResult, AsanaConfig>(
  async (input, context) => {
    ...
    for (const change of orderedChanges) {
      ...
      try {
        let result: AsanaSyncResult | null = null;

        if (change.entityType === 'todo') {
          result = await syncTodoToAsana(
            {
              event: `todo.${op}`,
          ...
          result = await syncTimesheetTaskToAsana(
            {
              event: `task.${op}`,
